...
class Grade extends Component<IProps, PageState> {
  ...
  getList = async (year: number, term: number, captcha = '') => {
    ...
    if (!this.props.grades?.grades?.length) {
      ...
      const { AVERAGE_GPA, AVERAGE_GRADE, OBLIGATORY_AVERAGE_GPA, grades } = this.props.grades
      ...
      const myGrades = grades.map((item) => {
        ...
        maxRemarkLength = itemObj.courseRemark.length > maxRemarkLength ? itemObj.courseRemark.length : maxRemarkLength
        return itemObj
      })
      ...
      const nextState: any = {
        ...
        OBLIGATORY_AVERAGE_GPA: `${OBLIGATORY_AVERAGE_GPA === 'NaN' ? '--' : OBLIGATORY_AVERAGE_GPA}`,
        myGrades,
        maxRemarkLength
      }
  ...
  // 跳转到弹窗页面
  toPingGu= () => {
    goWebviewPage('https://mp.weixin.qq.com/s?__biz=MzUwOTk3NTEzNg==&mid=2247483740&idx=1&sn=f65b45ddfe6e6429a54864f4f7074d4c&chksm=f90b4e53ce7cc7456f6ec8d3e8c8db309092001f62f51fe1df89610ef86ebbcac543c734ec50&token=2073460005&lang=zh_CN#rd', '去评教')
  }
    ...
    return (
      <View className="grade-page">
        ...
        <View className="container">
          ...
          {
            showEmpty && <View className="empty">
              <Text className="select-text">还没有这学期的成绩</Text>
            </View>
          }
          {
            !showEmpty && <View className="grade-list">
              {
                showGDP && <View className="total-wrapper">
                    ...
                    myGrades.map((item, index) => {
                      ...
                      >
                        ...
                        <View className="list-item passSign">
                          <Text className="list-text" style={`color:${passSignStyle[item.passSign]}`}>{item.passSign}</Text>
                        </View>
                        ...
                        <View className="list-item courseCharacter">
                          <Text className="list-text" >{item.courseCharacter}</Text>
                        </View>
                      </View>
                    })
                  }
        ...
        {/* 新手引导 */}
        {
          isFirstFull && <View className="tip-modal" onClick={this.closeTipModal}>
